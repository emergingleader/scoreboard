...
// ── Logo component (replaces base64 image) ───────────────────────────────────
function Logo({ dark }) {
  const textColor  = dark ? EL.dark      : EL.white;
  const subColor   = dark ? EL.greyText  : "rgba(255,255,255,0.65)";
  return (
    <div style={{ display:"flex", alignItems:"center", gap:"9px" }}>
      {/* Feet icon SVG matching EL brand */}
      <svg width="36" height="30" viewBox="0 0 36 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        {/* Left foot sole */}
        <ellipse cx="11" cy="19" rx="6.5" ry="9" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        {/* Right foot sole */}
        <ellipse cx="24" cy="19" rx="6.5" ry="9" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        {/* Left toes */}
        <circle cx="5"  cy="9"  r="2.2" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        <circle cx="9"  cy="6"  r="2.5" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        <circle cx="13.5" cy="5" r="2.5" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        {/* Right toes */}
        <circle cx="18" cy="9"  r="2.2" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        <circle cx="22" cy="6"  r="2.5" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
        <circle cx="26.5" cy="5" r="2.5" fill={dark ? EL.mid : "rgba(255,255,255,0.9)"} />
      </svg>
      <div>
        <div style={{ fontSize:"15px", fontWeight:"700", color:textColor, fontFamily:"'DM Sans', sans-serif", letterSpacing:"-0.2px", lineHeight:1.2 }}>
          Emerging Leaders
        </div>
        <div style={{ fontSize:"9.5px", color:subColor, fontWeight:"400", letterSpacing:"0.2px", lineHeight:1 }}>
    ...
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(26,13,5,0.9)", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px" }}>
      ...
      <div style={{ background:EL.white, borderRadius:"18px", padding:"28px 24px", width:"100%", maxWidth:"340px", position:"relative", boxShadow:"0 8px 40px rgba(0,0,0,0.4)" }}>
        <div style={{ textAlign:"center", marginBottom:"22px" }}>
          <div style={{ display:"flex", justifyContent:"center", marginBottom:"14px" }}><Logo dark /></div>
      ...
      <div style={{ background:EL.dark, padding:"16px 16px 12px", position:"sticky", top:0, zIndex:50, boxShadow:`0 3px 16px ${EL.brownDeep}88`, overflow:"hidden" }}>
        ...
        <div style={{ position:"relative" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"12px" }}>
            <div>
              <Logo />
              <div style={{ fontSize:"9px", color:"rgba(255,255,255,0.45)", fontWeight:"700", letterSpacing:"2px", marginTop:"4px" }}>STRATEGY SCOREBOARD · 2026–2027</div>
            </div>
            {currentUser && (
